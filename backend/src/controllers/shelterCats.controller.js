...
export async function getAllAvailableCats(req, res) {
  try {
    ...
    if (search && search.trim()) {
      // Join with tags to enable tag search
      conditions.push(
        `(
      name LIKE ? OR 
      breed LIKE ? OR 
    ...
    const featuredCats = allCats.filter(
      (cat) => cat.source === "featured_foster",
    );
    const partnerCats = allCats.filter(
      (cat) => cat.source === "partner_foster",
    );
  ...
  try {
    ...
    res.json({
      ...
      needs_refresh:
        info.total === 0 ||
        (info.oldest_update &&
          new Date() - new Date(info.oldest_update) > 7 * 24 * 60 * 60 * 1000),
    });
  } catch (error) {
